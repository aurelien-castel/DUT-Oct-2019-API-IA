...
class Plateau():
    ...
    def choisirDeplacement(self, joueur):
        """Méthode choisirDeplacement

        Demande au joueur s'il veut déplacer son pion.
        """
        ...
        if(choix == 'o' or choix == 'O'):
            ...
            # Le joueur s'est déplacé et ne pourra donc pas poser de mur
            return True

        # Si le joueur entre 'n' ou 'N'
        elif(choix == 'n' or choix == 'N'):
            print("Votre choix a bien été enregistré.")
            return False

        # Si le joueur entre autre chose
        else:
            print("Réponse invalide, veuillez recommencer...")
            self.choisirDeplacement(joueur)


    def choisirPositionMur(self, joueur):
        """Méthode choisirPositionMur

        Vérifie que le joueur possède encore des murs à poser.
        Si c'est le cas, lui demande la position du mur à poser.
        """
        ...
        # Rappel au joueur
        print("Vous aviez choisi de poser un mur.\n")

        # S'il reste des murs à poser au joueur
        if(joueur.nbMurs > 0):
            murs = self.numMurs()
            num = int(input("Donnez le numéro de la position du mur à poser : "))

            # Si le joueur entre l'une des positions possibles
            if(num < len(murs)+1):
                joueur.poserMur(murs, num, self)
            
            # Sinon
            else:
                print("Cette position n'existe pas, veuillez recommencer...")
                self.choisirPositionMur(joueur)

        # S'il ne lui reste aucun mur à poser
        else:
            print("Vous êtes obligé de vous déplacer, il ne vous reste aucun mur à poser...")
            self.choisirDeplacement(joueur)


    def tour(self, nature, action):
        """Méthode tour

        Lance un tour : les deux joueurs se déplacent, posent un mur ou ne font rien :
            - selon la nature du tour (pair ou impair),
            - et le choix qu'ils ont fait au tour précédent.
        """

        # Nombre de joueurs
        nbJoueurs = 2

        # Tableau des deux joueurs
        tabDesJoueurs = [self.j1, self.j2]

        # Si le tour est pair
        if(nature % 2 == 0):

            # On demande à chaque joueur s'il veut déplacer son pion
            for i in range(0, nbJoueurs):
                action[i] = self.choisirDeplacement(tabDesJoueurs[i])
        
        # Si le tour est impair
        else:

            # On demande aux joueurs qui n'ont pas déplacé leur pion s'il veulent poser un mur
            for i in range(0, nbJoueurs):
                if(action[i] is False):
                    self.choisirPositionMur(tabDesJoueurs[i])
            
            # On réinitialise le tableau action à False pour le prochain tour
            for i in range(0, nbJoueurs):
                action[i] = False
        ...
        # On retourne le tableau action pour qu'au prochain tour impair :
            # - on sache si le joueur peut poser un mur,
            # - ou s'il a déjà fait une action (ie s'est déplacé).
        return action
